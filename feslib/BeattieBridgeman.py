"""
Beattie-Bridgeman equation of state for fluid mixtures
"""
...
def phi0(v,T):
  """
  Macier-Planck potential without function K(T)
  """
  # RT = R*T
...
def phi0_v(v,T):
  """
  derivative of the Macier-Planck potential with respect to v
  """
  return p0(v,T)/T
...
def phi0_vv(v,T):
  """
  2nd derivative of the Macier-Planck potential with respect to v
  """
...
def Q1(v):
  """
  coefficient for calculating T from the equation phi_vv = 0
  """
...
def PhTrEq1(v1,v2,T):
  """
  First phase transition equation
...
def PhTrEq2(v1,v2,T):
  """
  second phase transition equation
  Gibbs chemical potential is maintained
  """
...
def PhTrEq12(vT,v2):
  """
  both phase transition equations
  this function is used in fsolve
  vT -- array, 0th coordinate -- v, 1st -- T
  """
...
def Q0(T):
  """
  coefficient for calculating the discriminant curve as the root of a cubic equation
  """
...
def p1(T):
  """
  discriminant curve (discriminant from p0 to v)
...
def PhTrSolve(T, p01, p02, v11, v12, v21, v22, eps1=1e-3, eps2=1e-3, dp=1e-2, dv=1e-3):
  """
  solving a system for a phase transition by brute force
  for given T and p we find values two value v
...
tt = time()
for i in range(nn):
  #we consider the right border for v1 and the left for v2
  #these boundaries are the maximums of the graph p0(v,TM1[i])
  p01 = p2(TM2[i])
  p02 = p3(TM2[i])
